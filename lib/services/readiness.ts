...
export async function getSystemReadiness(): Promise<ReadinessCheck[]> {
  const checks: ReadinessCheck[] = [
    ...
    { label: "WhatsApp monitor cron", ok: isConfiguredValue(process.env.CRON_SECRET), detail: isConfiguredValue(process.env.CRON_SECRET) ? "Cron endpoint protected" : "Add CRON_SECRET before enabling cron" },
