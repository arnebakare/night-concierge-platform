...
export function SalaryReport({
  ...
  return (
    <LuxuryCard className="mt-4 salary-report bg-white text-slate-950">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.22em] text-champagne-700">Salary report</p>
          <h2 className="mt-1 text-xl font-semibold">Printable table-cost sheet</h2>
          <p className="mt-1 text-sm text-slate-500">Use the date filters above, adjust dates or table costs, then print or save as PDF.</p>
        </div>
        <Button type="button" variant="secondary" className="bg-slate-100 text-slate-900 hover:bg-slate-200 print:hidden" onClick={() => window.print()}>
          <Printer className="size-4" />
          Print report
        </Button>
      </div>

      <div className="mt-4 grid grid-cols-2 divide-x divide-y divide-slate-200 overflow-hidden rounded-md border border-slate-200 md:grid-cols-4 md:divide-y-0">
      ...
      <div className="mt-3 grid gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 print:hidden sm:grid-cols-[1fr_auto] sm:items-center">
        <div>
          <p className="text-sm font-semibold">Commission estimate</p>
          <p className="text-xs text-slate-500">{commissionRules.some((rule) => rule.active) ? "Using active commission rules. Manual rate applies where no rule matches." : "Change the percentage for this report before printing."}</p>
        </div>
        <label className="grid grid-cols-[auto_5.5rem] items-center gap-2 text-sm">
          ...
          <input
            ...
            inputMode="decimal"
            className="h-10 rounded-md border border-slate-200 bg-white px-2 text-right text-slate-950"
              ...
              <tr key={request.id} className="border-t border-slate-200">
                <td className="p-3">
                  <input
                    ...
                    className="h-10 w-36 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-950 print:border-0 print:bg-transparent print:p-0"
                  />
                </td>
                <td className="p-3">
                  <p className="font-semibold text-slate-950">{request.clients?.name ?? "Client"}</p>
                  <p className="text-xs text-slate-500">{request.clients?.phone ?? ""}</p>
                </td>
                ...
                <td className="p-3">
                  <form id={`table-cost-${request.id}`} action={updateRequestTableCost}>
                    ...
                    <input
                      ...
                      placeholder="0"
                      className="h-10 w-28 rounded-md border border-slate-200 bg-white px-2 text-sm text-slate-950 print:border-0 print:bg-transparent print:p-0"
                    />
                  </form>
                </td>
                <td className="p-3 text-xs text-slate-500">{rule ? rule.label ?? `${Number(rule.rate_percent).toFixed(1)}% + ${formatCurrency(rule.flat_fee_cents / 100)}` : `${commissionRate}% default`}</td>
                <td className="p-3 print:hidden">
                  <StatusSubmitButton label="Save" pendingLabel="Saving" form={`table-cost-${request.id}`} size="sm" variant="secondary" className="bg-slate-100 text-slate-900 hover:bg-slate-200" />
                </td>
              </tr>
              );
            }) : (
              <tr>
                <td className="p-6 text-center text-slate-500" colSpan={9}>No confirmed bookings in this period.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="mt-4 flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 p-3">
        <span className="flex items-center gap-2 text-sm font-semibold">
          <Calculator className="size-4 text-champagne-700" />
          Salary report total
        </span>
        <span className="text-right">
          <span className="block text-xs text-slate-500">Table cost {formatCurrency(totalCost)}</span>
          <span className="block text-2xl font-semibold text-slate-950">{formatCurrency(commission)}</span>
        </span>
      </div>
    </LuxuryCard>
  );
