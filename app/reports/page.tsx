...
export default async function ReportsPage({ searchParams }: Readonly<{ searchParams: Promise<{ from?: string; to?: string; club?: string }> }>) {
  ...
  return <AppShell profile={profile} title="Performance" eyebrow="Live reporting">
    <form action="/reports" className="mb-4 grid gap-2 rounded-lg border border-slate-200 bg-white p-2.5 md:grid-cols-[1fr_1fr_1.4fr_auto]"><Input name="from" type="date" defaultValue={filters.from ?? ""} aria-label="From date" className="border-slate-200 bg-white text-slate-950" /><Input name="to" type="date" defaultValue={filters.to ?? ""} aria-label="To date" className="border-slate-200 bg-white text-slate-950" /><select name="club" defaultValue={filters.club ?? ""} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm text-slate-950"><option value="">All venues</option>{clubs.map((club) => <option key={club.id} value={club.id}>{club.name}</option>)}</select><Button type="submit">Apply</Button></form>
