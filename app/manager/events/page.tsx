  ...
  return (
    <AppShell profile={profile} title="Events" eyebrow="Programming">
      <LuxuryCard>
        ...
        <div className="mt-4 compact-list grid gap-2">
          {latestRuns.slice(0, 8).map((run) => (
            <div key={run.id} className="client-row rounded-md border border-champagne-700/30 bg-background/40 p-3">
      ...
      <div className="compact-list grid gap-2">
        {events.map((event) => (
          <LuxuryCard key={event.id} className={`client-row ${!event.active ? "opacity-70" : ""}`}>
            <div className="grid gap-2 md:grid-cols-[minmax(180px,1fr)_auto] md:items-start">
              ...
              <span className={event.active ? "text-xs font-semibold text-emerald-400" : "text-xs text-muted-foreground"}>{event.active ? "ACTIVE" : "ARCHIVED"}</span>
            </div>
