...
import { getServicePathDefaultsForProfile, getServiceRoutingRulesForProfile, getServiceRoutingStatsForProfile, getTeamPromoters } from "@/lib/data/app";

export default async function ManagerRoutingPage() {
  const profile = await requireProfile(["PROMOTER_MANAGER", "SUPER_ADMIN"]);
  const [rules, promoters, stats, serviceDefaults] = await Promise.all([
    getServiceRoutingRulesForProfile(profile),
    getTeamPromoters(profile.id),
    getServiceRoutingStatsForProfile(profile),
    getServicePathDefaultsForProfile()
  ]);
  ...
  return (
    <AppShell profile={profile} title="Routing" eyebrow="Service handling">
      ...
      <ServiceRoutingPanel rules={rules} serviceDefaults={serviceDefaults} promoters={promoters} managers={[profile]} stats={stats} />
    </AppShell>
  );
}
