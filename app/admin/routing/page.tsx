...
import { getServicePathDefaultsForProfile, getServiceRoutingRulesForProfile, getServiceRoutingStatsForProfile, getUsersForAdmin } from "@/lib/data/app";

export default async function AdminRoutingPage() {
  const profile = await requireProfile(["SUPER_ADMIN"]);
  const [rules, users, stats, serviceDefaults] = await Promise.all([
  ...
  return (
    <AppShell profile={profile} title="Routing" eyebrow="Concierge operations">
      ...
      <ServiceRoutingPanel rules={rules} serviceDefaults={serviceDefaults} promoters={promoters} managers={managers} stats={stats} />
    </AppShell>
  );
}
