  ...
  return (
    <AppShell profile={profile} title="Users" eyebrow="Admin">
      <LuxuryCard className="mb-4">
        <details>
          <summary className="cursor-pointer text-sm font-semibold">Create user</summary>
        <form action={createUserProfile} className="mt-3 grid gap-3 md:grid-cols-2">
      ...
      <div className="compact-list grid gap-2">
        {users.map((user) => (
          <LuxuryCard key={`${user.id}-${user.role}`} className={!user.active ? "opacity-70" : undefined}>
            ...
            {user.id !== profile.id && (
              <details className="mt-2 rounded-md border border-champagne-700/30 bg-secondary/60 px-3 py-2">
                <summary className="cursor-pointer text-sm font-medium text-champagne-300">Manage</summary>
